...
# Try CUDA extension
try:
    from .kernels.cauchy import cauchy_mult as cauchy_cuda
    from .kernels.vandermonde import log_vandermonde_cuda
...
class S4Layer(nn.Module):
    def __init__(self, args): 
        super().__init__()       
        self.s4_1, self.s4_2 = [S4Block(
                    d_model=args.pred_dim,
                    d_state=64,
                    channels=1,
                    bidirectional=True,
                    activation="gelu",
        ...
        self.ln_1 = nn.LayerNorm(args.pred_dim)
        self.ln_2 = nn.LayerNorm(args.pred_dim)
        self.ff_1 = nn.Linear(args.pred_dim, args.pred_dim*4)
        self.ff_2 = nn.Linear(args.pred_dim*4, args.pred_dim)
        self.activation = nn.GELU()

    # Note: True is allowd to attend
    def forward(self, x, src_pad_mask):
        out = self.ln_1(x)
        out = out * src_pad_mask
        out, _ = self.s4_1(out)
        x = x + out
        out = self.ln_2(x)
        out = out * src_pad_mask
